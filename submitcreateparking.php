...
$dbname = "parking_db"; // Just the database name, not the table name
...
if (isset($_POST['location']) && isset($_POST['spaces'])) {
    // Get form data
    $location = $_POST['location'];
    $spaces = $_POST['spaces'];

    // Prepare and bind
    $stmt = $conn->prepare("INSERT INTO parking_spaces (location, spaces) VALUES (?, ?)");
    $stmt->bind_param("si", $location, $spaces); // 's' for string, 'i' for integer
    ...
    echo "Please provide location and number of spaces.";
}
...
<h2>View Created Parking Spaces</h2>
...
$sql = "SELECT id, location, spaces FROM parking_spaces";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo "<table><tr><th>ID</th><th>Location</th><th>Spaces</th></tr>";
    // Output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>" . $row["id"] . "</td><td>" . $row["location"] . "</td><td>" . $row["spaces"] . "</td></tr>";
    }
    echo "</table>";
} else {
...
<p><a href="createparkingspace.php"><button>Back to Home</button></a></p>
