...
// Fetch existing data
$idURL = $_GET['user_id']; // Ensure this matches the id parameter passed in the URL

$query = "SELECT * FROM registration WHERE user_id = '$idURL'";
$result = mysqli_query($conn, $query);
$row = mysqli_fetch_assoc($result);

// Store fetched data into variables for pre-populating form fields
$idURL = $row["user_id"];
$name= $row["user_name"];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $plateNumber = $_POST["vehicle_plate"];
    ...
    $idURL = mysqli_real_escape_string($conn, $idURL);
    $name = mysqli_real_escape_string($conn,$name);
    $password = mysqli_real_escape_string($conn, $password);
    $plateNumber = mysqli_real_escape_string($conn, $plateNumber);
    $vtype = mysqli_real_escape_string($conn,$vtype);
    $email = mysqli_real_escape_string($conn,$email);
    $utype = mysqli_real_escape_string($conn,$utype);
    $address = mysqli_real_escape_string($conn,$address);
    
    // Update query
    $query = "UPDATE registration SET user_name='$name', user_pw='$password', vehicle_plate='$plateNumber', vehicle_type='$vtype', user_email='$email', user_type='$utype', user_address='$address' WHERE user_id='$idURL'";
    
    // Execute query
    $result = mysqli_query($conn, $query);
    
    if ($result) {
        // Redirect to view booking page after update
...
<article>
  ...
  <form method="post" action="regsubmit.php">
    ...
    <input type ="text" name="nama" size="40" value="<?php echo $name; ?>">
    ...
    <input type="text" id="staff_id" name="staff_id" value="<?php echo $idURL; ?>" required>

    <label for="address">Address:</label>
    <input type="text" id="user_id" name="user_address" value="<?php echo $address; ?>" required>
